      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex p-1 bg-line/20 rounded-lg shrink-0 w-fit">
           <button 
             ...
             className={`px-4 py-2 text-sm font-medium rounded-md transition-all cursor-pointer ${activeDataset === "original" ? "bg-card text-brand border border-line shadow-sm" : "text-muted hover:text-ink hover:bg-card/50"}`}
           >
             Videos Originales
           </button>
           <button 
             onClick={() => setActiveDataset("recortados")}
             className={`px-4 py-2 text-sm font-medium rounded-md transition-all cursor-pointer ${activeDataset === "recortados" ? "bg-card text-brand border border-line shadow-sm" : "text-muted hover:text-ink hover:bg-card/50"}`}
           >
             Videos Recortados
           </button>
        </div>
